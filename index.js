const { LICENSEGEN_PUBLIC_KEY } = process.env
...
async function main() {
  ...
  switch (scheme) {
    ...
    case 'ED25519_SIGN': {
      ...
      const der = Buffer.from(LICENSEGEN_PUBLIC_KEY, 'base64')
    ...
    case 'RSA_2048_PKCS1_SIGN_V2': {
      ...
      const ok = verifier.verify({ key: LICENSEGEN_PUBLIC_KEY, padding: crypto.constants.RSA_PKCS1_PADDING }, sig, 'base64')
      if (ok) {
        console.log(chalk.green(`License key is cryptographically valid!`))
        console.log(chalk.gray(`Decoded: ${dec}`))
      } else {
    ...
    case 'RSA_2048_PKCS1_PSS_SIGN_V2': {
      ...
      const ok = verifier.verify({ key: LICENSEGEN_PUBLIC_KEY, padding: crypto.constants.RSA_PKCS1_PSS_PADDING }, sig, 'base64')
      if (ok) {
        console.log(chalk.green(`License key is cryptographically valid!`))
        console.log(chalk.gray(`Decoded: ${dec}`))
      } else {
    ...
    case 'RSA_2048_PKCS1_ENCRYPT': {
      ...
      // Decrypt the key
      try {
        const decryptedKey = crypto.publicDecrypt({ key: LICENSEGEN_PUBLIC_KEY, padding: crypto.constants.RSA_PKCS1_PADDING }, buf)
    ...
    case 'RSA_2048_JWT_RS256': {
      try {
        const dec = jwt.verify(key, LICENSEGEN_PUBLIC_KEY, { algorithms: ['RS256'] })
